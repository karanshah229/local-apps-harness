...
2. Read `.agents/references/recipe-contracts.md` and the selected `templates/*/recipe.json`; invoke `review-stack-exception` only if no recipe can meet a concrete requirement.
...
4. Create only requested product behavior. Add the technical application contract from `.agents/references/app-contract.md`.
...
Complete when the requested journeys work from the registered access URL or mobile artifact and no operational edge is missing.
