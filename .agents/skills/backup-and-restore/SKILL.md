...
description: Back up, verify, retain, or restore registered PostgreSQL or legacy SQLite data, uploads, and local operational configuration without exporting local secrets.
---
...
5. The built-in filesystem handler restores registered legacy SQLite data, creates a safety backup first, scopes container changes to one service, checks health, and rolls back on failure. A PostgreSQL app must register and implement its dump/restore handler before use; never substitute file copying.
