...
export default function TaskMainView({
	...
	return (
		<main className="flex-1 flex flex-col h-full overflow-hidden bg-background">
			...
			<div
				className={cn(
					"text-white px-4 pt-4 pb-4 sm:px-6 sm:py-6 transition-all duration-300 shadow-md flex-shrink-0",
					getThemeGradient(),
				)}
			>
				...
				{isMultiSelectMode && selectedTaskIds.length > 0 && (
					<div className="sticky top-0 z-20 flex items-center justify-between p-3 rounded-2xl bg-primary text-primary-foreground shadow-lg animate-in slide-in-from-top-2">
						...
						>
							...
							<span>
								Send WhatsApp Digest ({selectedTaskIds.length})
							</span>
						</Button>
					</div>
				)}
				...
				{tasks.length === 0 ? (
					<div className="flex flex-col items-center justify-center py-16 px-4 text-center text-muted-foreground">
						...
						<p className="text-xs text-muted-foreground mt-1 max-w-xs">
							Stay organized by adding tasks above or assigning
					...
					tasks.map((task) => {
						...
						const isCheckedForBatch = selectedTaskIds.includes(
							task.id,
						);
