...
export function useThrottledTaskAutosave({
  ...
  const [status, setStatus] = useState(enabled ? 'saved' : 'draft');
  const [isSlowSaving, setIsSlowSaving] = useState(false);
  const [feedbackType, setFeedbackType] = useState(null); // 'loading' | 'success' | 'error' | null
  const [showFeedback, setShowFeedback] = useState(false);

  const pendingUpdatesRef = useRef(null);
  const failedUpdatesRef = useRef(null);
  const queuedTaskIdRef = useRef(taskId);
  ...
  const invokeRef = useRef(async () => undefined);
  const scheduleRef = useRef(() => undefined);
...
export function getTaskAutosaveLabel(status) {
  switch (status) {
    case 'saving':
      return 'Saving…';
    case 'error':
